...
export default function SearchFilters({ initial = {} }) {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("ll3_members_filters_collapsed");
      if (saved != null) {
        setCollapsed(saved === "1");
      } else {
        // default collapsed on small screens
        setCollapsed(window.matchMedia("(max-width: 480px)").matches);
  ...
  useEffect(() => {
    try {
      localStorage.setItem("ll3_members_filters_collapsed", collapsed ? "1" : "0");
    } catch { }
  }, [collapsed]);
  ...
  return (
    <div className="ll3-membersFilters" aria-label="Search filters">
      <div className="ll3-control__header">
        ...
        <div className="ll3-control__headerRight">
          <button
            type="button"
            className="ll3-linkbtn"
      ...
      {!collapsed && (
        <>
          <div className="ll3-filters ll3-membersFilters__grid">
            {/* NAME */}
              ...
              <form onSubmit={onSubmit}>
                <div className="ll3-searchbox">
                  ...
                  {open && !loading && (stateMatches.length || memberMatches.length) ? (
                    <div id="searchbox-listbox" className="ll3-searchbox__dropdown" role="listbox">
                          ...
                          {stateMatches.map((s) => (
                            <div key={s.stateCode || s.state} className="ll3-sbState">
                              <div className="ll3-sbState__head">
                                {s.state}
                                {s.stateCode ? <span className="ll3-sbState__code">({s.stateCode})</span> : null}
                              </div>
                      ...
                      {stateMatches.length > 0 && memberMatches.length > 0 && <hr className="ll3-sbDivider" />}
                  ...
                  {open && !loading && stateMatches.length === 0 && memberMatches.length === 0 && (
                    <div className="ll3-searchbox__dropdown ll3-searchbox__empty">No matches.</div>
                  )}
                  {open && loading && <div className="ll3-searchbox__dropdown ll3-searchbox__empty">Loading…</div>}
                </div>
              </form>
            </div>
            ...
            {/* PARTY */}
            <div className="ll3-field">
              <label className="ll3-label">Party</label>
              <select className="ll3-input" value={current.party} onChange={(e) => setParam("party", e.target.value)}>
            ...
            {/* STATE */}
            <div className="ll3-field">
              <label className="ll3-label">State</label>
              <select
                className="ll3-input"
                value={current.state}
            ...
            <div className="ll3-actions ll3-membersFilters__actions">
          ...
          <div className="ll3-filterChips" role="status" aria-live="polite">
            {current.q && <Chip className="ll3-filterChip" label={`Name: ${current.q}`} onClear={() => setParam("q", "")} />}
            {current.chamber && (
              <Chip className="ll3-filterChip" label={`Chamber: ${current.chamber}`} onClear={() => setParam("chamber", "")} />
            )}
            {current.party && (
              <Chip className="ll3-filterChip" label={`Party: ${partyLabel(current.party)}`} onClear={() => setParam("party", "")} />
            )}
            {current.state && (
              <Chip className="ll3-filterChip" label={`State: ${current.state}`} onClear={() => setParam("state", "")} />
            )}
