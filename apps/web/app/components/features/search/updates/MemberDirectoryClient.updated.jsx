...
import MemberDirectoryRow from "@/app/components/features/search/updates/MemberDirectoryRow.updated";
...
function filterMembers(members, { q, chamber, party, seatStatus, districtFilter }) {
    ...
    return members.filter((m) => {
        ...
        const matchesSeatStatus =
            !normalizedSeatStatus || normalizeSeatStatus(m?.seatStatus) === normalizedSeatStatus;
        const matchesDistrict =
            !districtFilter || String(m?.district) === String(districtFilter);

        return matchesQuery && matchesChamber && matchesParty && matchesSeatStatus && matchesDistrict;
    });
}
...
export default function MemberDirectoryClient({ initialData }) {
    ...
    const [stateFilter, setStateFilter] = useState(() =>
        String(searchParams.get("state") || "").toUpperCase()
    );
    const [seatStatus, setSeatStatus] = useState(() => searchParams.get("seatStatus") || "");
    ...
    useEffect(() => {
        const nextSeatStatus = searchParams.get("seatStatus") || "";
        const nextState = String(searchParams.get("state") || "").toUpperCase();
    ...
    useEffect(() => {
        ...
        setActiveState((prev) => prev || stateFilter || nextStates?.[0]?.stateCode || "");
    }, [initialData, stateFilter]);

    useEffect(() => {
        document.body.classList.toggle("ll3-memberRefineIsOpen", isRefineOpen);
        return () => document.body.classList.remove("ll3-memberRefineIsOpen");
    }, [isRefineOpen]);

    useEffect(() => {
        function onKeyDown(event) {
            if (event.key === "Escape") setIsRefineOpen(false);
        }
    ...
    const filteredStates = useMemo(() => {
        return statesList
            .filter((state) => !stateFilter || state.stateCode === stateFilter)
            .map((state) => {
    ...
    }, [statesList, q, chamber, party, seatStatus, stateFilter, districtFilter]);
        ...
        <section className="ll3-membersDirectory" ref={topRef}>
                ...
                <div className="ll3-directoryFilters" aria-label="Directory filters">
                    <div className="ll3-directoryFilters__header">
                        <div className="ll3-directoryFilters__intro">
                            <h2 className="ll3-h2">Members</h2>
                            <p className="ll3-muted">
                                Search, sort, and filter current members and House seats.
                            </p>
                        </div>
                        ...
                        <button
                            type="button"
                            className="ll3-memberRefineButton"
            ...
            <div className="ll3-mobileStateBarWrap">
                <div className="ll3-mobileStateBar" aria-label="Mobile state navigation">
                    ...
                    <button type="button" className="ll3-mobileStateBar__topBtn" onClick={scrollToTop}>
                        Top
                    </button>
                </div>
            </div>
            ...
            {isRefineOpen ? (
                <div className="ll3-memberRefine" role="dialog" aria-modal="true" aria-label="Refine members">
                    <button
                        type="button"
                        className="ll3-memberRefine__backdrop"
                        aria-label="Close refine filters"
                        onClick={() => setIsRefineOpen(false)}
                    />

                    <div className="ll3-memberRefine__sheet">
                        <div className="ll3-memberRefine__head">
                            <div>
                                <h2 className="ll3-memberRefine__title">Refine members</h2>
                                <p className="ll3-memberRefine__sub">
                                    {totalVisibleRows} row{totalVisibleRows === 1 ? "" : "s"} shown
                                </p>
                            </div>
                            ...
                            <button
                                type="button"
                                className="ll3-memberRefine__close"
                                onClick={() => setIsRefineOpen(false)}
                                aria-label="Close refine filters"
                            >
                                <X size={18} />
                        ...
                        <button
                            type="button"
                            className="ll3-btn ll3-btn--full ll3-memberRefine__done"
                            onClick={() => setIsRefineOpen(false)}
                        >
                            Show results
                        </button>
