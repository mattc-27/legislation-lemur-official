...
export default function TopicDonut({ data, groups = [], onSelectTopic, mode = "page" }) {
  const rows = useMemo(() => {
    if (Array.isArray(data) && data.length) {
      return data
        .map((d) => ({ label: normalizeTopicLabel(d?.label), value: Number(d?.value) || 0 }))
        .filter((r) => r.value > 0);
    }
  ...
  const legendDefaultCount = isPanel ? 5 : 5;
  ...
  return (
    <div className={`viz viz--donut ${isPanel ? "viz--donutPanel" : "viz--donutPage"}`} role="group" aria-label="Topic mix">
        ...
        <div className="viz-donut__chart">
          <svg className="viz-donut__svg" viewBox="0 0 120 120" aria-hidden="true">
            <g transform="translate(60,60)">
              <circle r={R} fill="none" stroke="var(--llm3-border, #E5E7EB)" strokeWidth="16" />
              {annotated.map((r, i) => {
              ...
              <text className="viz-donut__centerText" x="0" y="7" textAnchor="middle">
                {total}
              </text>
            </g>
          </svg>
        ...
        <ul className="viz__legend">
            ...
            return (
              ...
              >
                <span className="viz__legend-label">
                  <Icon className="viz__legend-icon" size={14} strokeWidth={2} aria-hidden="true" style={{ color }} />
                  {topicMeta.short || r.label}
                </span>
          ...
          {annotated.length > legendDefaultCount && (
            <li className="viz__legendToggleRow">
              <button type="button" className="viz__legend-more" onClick={() => setShowAllLegend((v) => !v)} aria-expanded={showAllLegend}>
