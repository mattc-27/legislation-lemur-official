...
export default async function MemberDirectoryPage() {
  const [directory, changes, freshness] = await Promise.all([getMembersDirectory(), getMemberRecentChanges({ limit: 12 }), getViewsFreshness(["mv_member_directory_v2", "v_house_seats_current_v1", "v_member_recent_changes_v3"])]);
  const asOfText = formatAsOfMMDDYYYY(freshness?.asOf);
  return (
    <ExplorerPageShell variant="members" className="ll3-members">
      <ExplorerPageHeader eyebrow="Representation explorer" title="Member Directory" activeTool="member" asOfText={asOfText} description="Browse Congress by state delegation, chamber, and vacant House seats. Use Smart Search for name or cross-entity searches." />
