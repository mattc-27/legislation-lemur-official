...
function toItem(r, kind) {
    const role = r.member_role || (r.my_role === "s" ? "sponsored" : r.my_role === "c" ? "cosponsored" : null);
    const kinds = Array.from(new Set([kind, role, r.my_role].filter(Boolean)));
    console.log("RAW BILL ID", r.bill_id);
    return {
        id: r.bill_id,
        billId: r.bill_id,
        bill_id: r.bill_id,
        ...
        url: r.url || r.congress_url,
        congressUrl: r.congress_url || r.url,
        congress_url: r.congress_url || r.url,
        bioguideId: r.bioguide_id,
        bioguide_id: r.bioguide_id,
        memberName: r.member_name,
        member_name: r.member_name,
        appHref: `/bills/${r.bill_id}`,
        app_href: `/bills/${r.bill_id}`,
        ...
        cosponsorCount: r.cosponsor_count,
        cosponsor_count: r.cosponsor_count,
        cosponsorsTotal: r.cosponsors_total,
        cosponsors_total: r.cosponsors_total,
        hasAiSummary: r.has_ai_summary,
...
export async function getMemberBills(
    ...
    FROM ${ACTIVE_VIEW_SCHEMA}.mv_member_bill_activity_v1
  ...
  `;
    ...
    const { rows } = await timed(
        ...
        () => q("member:getBills:mvBillActivity", sql, [bioguideId, congress, limit, offset]),
...
export async function getMemberSponsoredLegislation(bioguideId, { max = 250 } = {}) {
  ...
  `;
    ...
    const { rows } = await timed(
        ...
        () => q("member:getSponsored:mvBillActivity", sql, [bioguideId, queryLimit]),
...
export async function getMemberCosponsoredLegislation(bioguideId, { max = 250 } = {}) {
  ...
  `;
    ...
    const { rows } = await timed(
        ...
        () => q("member:getCosponsored:mvBillActivity", sql, [bioguideId, queryLimit]),
        { bioguideId, max, queryLimit }
    );
